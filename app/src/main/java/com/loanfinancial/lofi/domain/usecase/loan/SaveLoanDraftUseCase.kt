 ...
 */
class SaveLoanDraftUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        val draft = LoanDraft(
            ...
            createdAt = id?.let { repository.getDraftById(it)?.createdAt ?: System.currentTimeMillis() } 
    ...
    suspend fun updateEmploymentInfo(
        ...
        val result = repository.updateEmploymentInfo(
            draftId, jobType, companyName, jobPosition, workDurationMonths, 
            workAddress, officePhoneNumber, declaredIncome, additionalIncome, npwpNumber
        )
        // Update currentStep
        if (result.isSuccess) {
    ...
    suspend fun updateDocumentPaths(
        ...
        return repository.updateDocumentPaths(draftId, documentPaths)
    }
}