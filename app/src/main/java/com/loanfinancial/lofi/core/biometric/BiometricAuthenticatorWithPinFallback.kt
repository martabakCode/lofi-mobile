...
class BiometricAuthenticatorWithPinFallback
    @Inject
    constructor(
        ...
        suspend fun authenticateWithFallback(): Flow<AuthResult> =
            flow {
                if (biometricAuthenticator.isBiometricAvailable() &&
                    ...
                    biometricAuthenticator
                        .authenticate("Authentication", "Use biometric to verify")
                        .collect { result ->
        ...
        suspend fun verifyPin(pin: String): Flow<PinVerificationResult> =
            flow {
