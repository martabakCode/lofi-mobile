...
class LoanSubmissionManagerImpl @Inject constructor(
    ...
    override suspend fun submitLoanOffline(loanData: LoanSubmissionData): Result<String> {
        return try {
            ...
            val entity = PendingLoanSubmissionEntity(
                loanId = loanId,
                userId = userId,
    ...
    override fun getPendingSubmissions(): Flow<List<PendingLoanSubmission>> {
        return kotlinx.coroutines.flow.flow {
             ...
             if (userId != null) {
                 emitAll(pendingSubmissionDao.getPendingSubmissionsByUser(userId).map { entities ->
                    entities.map { entity ->
    ...
    override suspend fun cancelSubmission(loanId: String) {
        ...
        if (count > 0) {
            // Cancel work
            androidx.work.WorkManager.getInstance(context).cancelAllWorkByTag("submit_$loanId")
        }
    }
