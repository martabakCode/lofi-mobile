...
class DocumentRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun queueDocumentUpload(
            ...
            return withContext(Dispatchers.IO) {
                try {
                    ...
                    // Compress if needed
                    val finalFilePath =
                        if (shouldCompress && file.length() > 1024 * 1024) { // > 1MB
                    ...
                    val userId =
                        dataStoreManager.getUserId() ?: return@withContext BaseResult.Error(
        ...
        override suspend fun getPendingUploads(loanDraftId: String): List<PendingDocumentUploadEntity> = pendingUploadDao.getPendingForDraft(loanDraftId)

        override fun getPendingUploadsFlow(loanDraftId: String): kotlinx.coroutines.flow.Flow<List<PendingDocumentUploadEntity>> = pendingUploadDao.getPendingForDraftFlow(loanDraftId)
