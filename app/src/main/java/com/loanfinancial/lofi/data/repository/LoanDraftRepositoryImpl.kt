...
class LoanDraftRepositoryImpl
    ...
    constructor(
        ...
        override fun getAllActiveDrafts(): Flow<List<LoanDraft>> =
            loanDraftDao.getAllActiveDrafts().map { entities ->
                entities.map { it.toDomain() }
            }

        override fun getDraftsByStep(step: DraftStep): Flow<List<LoanDraft>> =
            loanDraftDao.getDraftsByStep(step.name).map { entities ->
                entities.map { it.toDomain() }
            }
        ...
        override suspend fun updateDraftStep(
            draftId: String,
            step: DraftStep,
        ) {
        ...
        override suspend fun getUnsyncedCompletedDrafts(): List<LoanDraft> = loanDraftDao.getUnsyncedCompletedDrafts().map { it.toDomain() }
