...
class DocumentUploadWorkerTest {
    ...
    @Test
    fun `doWork should retry when some uploads fail with retryable error`() =
        runTest {
            // Arrange
            val upload =
                createPendingUpload("upload_1", DocumentUploadStatus.PENDING).copy(
                    retryCount = 0,
                )
    ...
    @Test
    fun `doWork should fail when max retries reached`() =
        runTest {
            // Arrange
            val upload =
                createPendingUpload("upload_1", DocumentUploadStatus.PENDING).copy(
    ...
    @Test
    fun `doWork should skip completed uploads`() =
        runTest {
            // Arrange
    ...
    @Test
    fun `doWork should use compressed file path when available`() =
        runTest {
            // Arrange
            val upload =
                createPendingUpload("upload_1", DocumentUploadStatus.PENDING).copy(
    ...
    @Test
    fun `doWork should process multiple documents with mixed results`() =
        runTest {
            // Arrange
