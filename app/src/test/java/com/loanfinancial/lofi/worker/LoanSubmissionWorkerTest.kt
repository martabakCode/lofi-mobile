...
class LoanSubmissionWorkerTest {
    ...
    @Test
    fun `doWork should return failure when no loanId provided`() = runTest {
        // Arrange
        every { params.inputData.getString(LoanSubmissionWorker.KEY_LOAN_ID) } returns null
        
        val testWorker = LoanSubmissionWorker(
            context = context,
            params = params,
            loanRepository = loanRepository,
            documentRepository = documentRepository,
    ...
    @Test
    fun `doWork should create loan and submit successfully`() = runTest {
        // Arrange
    ...
    @Test
    fun `doWork should handle loan creation failure and retry`() = runTest {
        // Arrange
        val pendingSubmission = createPendingSubmission()
    ...
    @Test
    fun `doWork should update existing server loan when serverLoanId exists`() = runTest {
        // Arrange
        val pendingSubmission = createPendingSubmission().copy(
    ...
    @Test
    fun `doWork should skip submit if loan already submitted`() = runTest {
        // Arrange
        val pendingSubmission = createPendingSubmission()
    ...
    @Test
    fun `doWork should handle max retry count and fail`() = runTest {
        // Arrange
        val pendingSubmission = createPendingSubmission().copy(
    ...
    @Test
    fun `doWork should queue documents for upload`() = runTest {
        // Arrange
        val pendingSubmission = createPendingSubmission()
        val createdLoan = createLoan("server_loan_123", "DRAFT")
    ...
    private fun createPendingSubmission() = PendingLoanSubmissionEntity(
        loanId = "test_loan_id",
        userId = "user_123",
