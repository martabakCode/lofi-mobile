...
class LoanDetailViewModelTest {
    ...
    @Test
    fun `loadLoan should emit success when loan loaded`() = runTest {
        // Arrange
        val loan = createLoan("loan_123", "DRAFT")
        coEvery { getLoanDetailUseCase("loan_123") } returns flowOf(
            Resource.Loading,
    ...
    @Test
    fun `loadLoan should emit error when loan fails to load`() = runTest {
        // Arrange
        coEvery { getLoanDetailUseCase("loan_123") } returns flowOf(
            Resource.Loading,
    ...
    @Test
    fun `loadLoan should emit error when loan data is null`() = runTest {
        // Arrange
    ...
    @Test
    fun `submitLoan should update submit state to loading then success`() = runTest {
        // Arrange
        val loan = createLoan("loan_123", "SUBMITTED")
        coEvery { submitLoanUseCase("loan_123") } returns flowOf(
            Resource.Loading,
            Resource.Success(loan)
        )
    ...
    @Test
    fun `submitLoan should update submit state to error on failure`() = runTest {
        // Arrange
        coEvery { submitLoanUseCase("loan_123") } returns flowOf(
            Resource.Loading,
    ...
    @Test
    fun `submitLoan should update uiState on success`() = runTest {
        // Arrange
    ...
    @Test
    fun `resetSubmitState should reset to idle`() = runTest {
        // Arrange
        val loan = createLoan("loan_123", "SUBMITTED")
        coEvery { submitLoanUseCase("loan_123") } returns flowOf(
            Resource.Success(loan)
    ...
    @Test
    fun `loadLoan should handle exception gracefully`() = runTest {
        // Arrange
